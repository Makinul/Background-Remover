...
class ImageSegmentHelper(
    ...
    val context: Context,
    private var imageSegmentListener: SegmentListener? = null
) {
    ...
    // non-existent object
    fun clearImageSegment() {
        imagesegmenter?.close()
        imagesegmenter = null
    }

    fun setListener(listener: SegmentListener) {
        imageSegmentListener = listener
    }

    fun clearListener() {
        imageSegmentListener = null
    }
    ...
    fun setupImageSegment() {
        ...
        when(currentModel) {
            ...
            MODEL_SELFIE_SEGMENTER -> {
                baseOptionsBuilder.setModelAssetPath(MODEL_SELFIE_SEGMENT_PATH)
            }
        ...
        try {
            ...
            imageSegmentListener?.onError(
                "Image segment failed to initialize. See error logs for details"
            )
            Log.e(
                TAG,
                "Image segment failed to load model with error: " + e.message
            )
    ...
    // returns the results asynchronously to the caller.
    fun segmentImage(mpImage: MPImage): ImageSegmenterResult? {
    ...
    private fun returnSegmentationResult(
        ...
        imageSegmentListener?.onResults(
            ResultBundle(
    ...
    companion object {
        ...
        const val MODEL_SELFIE_SEGMENT_PATH = "selfie_segmenter.tflite"
